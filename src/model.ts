...
export interface IReactSnapOptions {
  ...
  concurrencyType:
    ...
    | typeof Cluster.CONCURRENCY_BROWSER;
  include?: string[];
  exclude?: RegExp[];
...
export interface ICrawlParams {
  ...
  publicPath?: string,
  sourceDir: string,
  beforeFetch(params: {page: Page, route: string}): any
  afterFetch(params: {page: Page, route: string, addToQueue: (newUrl: string) => Promise<void>, logs: string[]}): any
  onEnd(): void
}
